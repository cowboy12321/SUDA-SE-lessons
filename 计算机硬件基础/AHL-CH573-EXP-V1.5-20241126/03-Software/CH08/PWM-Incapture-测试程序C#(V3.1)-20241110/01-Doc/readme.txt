...
【20240901】陈裕庆
解决了打开串口程序崩溃的问题
    原因：当chart绘画时，会对使用的数据生成一个迭代器，此时若是使用多线程，其他线程可能对使用的数据进行更改
          此时，迭代器就会报错。本程序是在定时器事件里进行图表数据的修改，在串口接收数据的时候打开定时器事件。
          程序运行过程是：打开串口，串口开始接收数据并打开定时器事件，在定时器事件中进行数据的更改，数据更改后图表自动进行绘制
          若是步骤冲突了，在图表进行绘制的时候串口接受了数据，恰好此时定时器事件对绘制的数据进行了更改，程序就会崩溃
          串口在关闭的时候没有对数据进行清空，因此在程序里第二次打开串口时数据进行了更改，程序必定崩溃
    解决方案：将定时器事件的打开代码从串口接收数据函数里改为打开串口函数里，在关闭串口代码里添加清空绘制数据的代码
              原程序有两个定时器，只用到了一个，把没用到的删除了

【20240902】陈裕庆
解决了打开串口程序崩溃的问题，解决了打开串口时关闭程序导致程序崩溃的问题
    原因：1.和上边的问题一样，仍然是多线程并发导致的非法修改致使程序崩溃，上面的修改并没有很好的解决此问题
          2.窗口关闭时UI线程的控件已经被销毁，此时资源并没有全部回收，若要是后台线程继续访问UI资源就会引发异常
    解决方案：1.整个系统大改，将可重入的定时器System.Timers.Timer改为不可重入的System.Windows.Forms.Timer
                从根本上杜绝了并发导致的问题，且后者提供的定时精度完全足够使用，将原本的直接修改series.point
                数据改为两个集合,使用chart本身的数据绑定进行绘图
              2.添加窗口关闭处理FrmSCI_FormClosing函数，进行额外处理避免程序异常