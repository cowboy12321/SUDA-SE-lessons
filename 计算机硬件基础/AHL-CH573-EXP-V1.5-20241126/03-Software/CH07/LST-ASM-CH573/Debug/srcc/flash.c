...
uint8_t flash_write(uint16_t sect,uint16_t offset,uint16_t N,uint8_t *buf)
{
    ...
    {
        //（3.1.1）先写入第一个扇区
        flash_write_sect(sect,offset,Flash_Sect_size-offset,buf);
        //（3.1.2）再写入第二个扇区
        flash_write_sect(sect+1,0,N-(Flash_Sect_size-offset),buf+(Flash_Sect_size-offset));
    }
    //（3.2）写入字节数不会跨扇区
    else
     {
...
//    flag = FLASH_ROM_WRITE(addr,buf,N);
...
uint8_t flash_Best(uint16_t sect,uint16_t offset,uint16_t N,uint8_t *buf)
{
    ...
    for(i = 0; i < N; i+=8)
    {
        ...
        uint32_t *addr1 = &temp_h;
        uint32_t *addr2 = &temp_l;
        //（4.3）在绝对地址(addr+i)，实现双字写入
        flash_write_DoubleWord((addr+i),addr2,addr1);
    }
    return 0;
}
...
uint8_t flash_isempty(uint16_t sect,uint16_t N)
{
    ...
    flag = FLASH_ROM_VERIFY(StartAddr,NULL,N);
