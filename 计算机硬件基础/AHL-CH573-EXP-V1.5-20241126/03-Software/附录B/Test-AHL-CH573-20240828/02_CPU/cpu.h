...
//（3）【因内核而更改】中断宏定义
#define ENABLE_INTERRUPTS       ({ __asm("li t0, 0x8"); __asm("csrs mstatus, t0"); rt_interrupt_leave(); })// 开总中断()
#define DISABLE_INTERRUPTS      ({rt_interrupt_enter();  __asm("li t0, 0x8"); __asm("csrc mstatus, t0"); })// 关总中断()

//#define ENABLE_INTERRUPTS       __asm volatile ("csrrs t3,mstatus, (0x88);" )    // 开总中断()
//#define DISABLE_INTERRUPTS      __asm volatile ("csrrc t3,mstatus, (0x88);" )      // 关总中断()
//（4）【固定】不优化类型别名宏定义
