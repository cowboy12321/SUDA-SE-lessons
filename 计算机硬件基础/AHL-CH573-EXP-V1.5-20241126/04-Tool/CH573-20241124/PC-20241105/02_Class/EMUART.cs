...
namespace cn.edu.suda.sumcu.iot
{
    ...
    public class EMUART
    {
        ...
        frm_UserUpdate emuart1 = new frm_UserUpdate();
        /// ------------------------------------------------------------------------------
        /// <summary>  
        /// 功    能: 构造函数                           
        /// </summary>                                                                     
        ...
        /// -----------------------------------------------------------------------------
        //public int findDevice(byte[] im,out string com, int baudRate)
        //{
        //    //（1）定义并初始化局部变量
        //    int i;
        ...
        //        byte[] old=System.Text.Encoding.Default.GetBytes("[Are you an emuart???]");
  
        //        if (!send(old, out tmpRecv, 20, 3)
        ...
        /// <returns>0：成功找到设备；1：未找到串口；2：找到串口，但是未找到设备</returns>                  
        /// -----------------------------------------------------------------------------
        public int findDevice( out string com, int baudRate)
        {
            //（1）定义并初始化局部变量
            int i;
            string s1 = string.Empty;    //初始化临时字符串（置空）;
        ...
        /// 功    能:通过串口名直接连接串口。                                  
        /// </summary>                                                                     
        /// <param name="com">参数串口名。                                        
        /// <param name="baudRate">波特率，默认值为9600</param>                                          
        /// <returns>0：成功找到设备；1：未找到串口；2：找到串口，但是未找到设备</returns>                  
        /// -----------------------------------------------------------------------------
        public int LinkDevice(string com, int baudRate = 9600)
        {
            //（1）定义并初始化局部变量
            string s1 = string.Empty;    //初始化临时字符串（置空）;
            byte[] tmpRecv = null;
            ...
            //（3.1）打开串口
            if (!open(com, baudRate))    //若打开失败，返回1
                return 1;
        ...
        public bool send(byte[] SendByteArray, out byte[] RecvData, int time = 500, int cnt = 1)
        {
            ...
            try
            {
                ...
                for (int j = 0; j < cnt; j++)
                {
                    ...
                    _Uartport.DataReceived -= new System.IO.Ports.SerialDataReceivedEventHandler(recv);//设置接收处理函数
                    System.Threading.Thread.Sleep(100);  //防止串口频繁发送致死   //todo
                    this.send(SendByteArray);           //发送数据
        ...
        public bool send1(byte[] SendByteArray, out byte[] RecvData, int time = 500, int cnt = 1)
        {
            //（1）定义并初始化局部变量
            int length;
            RecvData = null;
            try
            {
                ...
                for (int j = 0; j < cnt; j++)
                {
                    //frm_UserUpdate.f1.textBox5.Text = "";
                    //frm_UserUpdate.f1.textBox5.Text += "1";
                    //frm_UserUpdate.f1.textBox5.Refresh();
                    //设置串口接收事件的触发条件为ReadBufferSize，目的是暂时关闭串口接收事件
                    _Uartport.DataReceived -= new System.IO.Ports.SerialDataReceivedEventHandler(recv);//设置接收处理函数
                    System.Threading.Thread.Sleep(40);  //防止串口频繁发送致死   //todo
                    this.send(SendByteArray);           //发送数据
                    ...
                    for (int i = 0; i < time; i++)
                    {
                        //frm_UserUpdate.f1.textBox5.Text += "2";
                        //System.Threading.Thread.Sleep(10);
                        byte[] data = read();
                        foreach (byte b in data)
                        {
                            //frm_UserUpdate.f1.textBox5.Text += "3";
                            //frm_UserUpdate.f1.textBox5.Refresh();
                            //若有正确数据返回，则说明数据接收成功
                            if ((length = CreateFrame(b, recvData)) != 0)   //组帧成功
                            {
                                //frm_UserUpdate.f1.textBox5.Text += "4";
                                string s = System.Text.Encoding.Default.GetString(recvData);
                                RecvData = new byte[length];    //创建数组
                                Array.Copy(recvData, 0, RecvData, 0, length);
                                this._Uartport.ReadExisting();   //读取已存在的字节
                                this._Uartport.DiscardInBuffer();
                                //frm_UserUpdate.f1.textBox5.Text += "5";
                                //this._Uartport.ReceivedBytesThreshold = 1; //打开串口接收事件
                                _Uartport.DataReceived += new System.IO.Ports.SerialDataReceivedEventHandler(recv);//设置接收处理函数
                                //frm_UserUpdate.f1.textBox5.Text += "6";
                                return true;
                            }
                        }
                    }
                    //StreamWriter sw = new StreamWriter(fs); // 创建写入流
                    //sw.WriteLine("{0},{1}\n", cnt, time); // 写入Hello World
                    //sw.Close(); //关闭文件
                }
                _Uartport.DataReceived += new System.IO.Ports.SerialDataReceivedEventHandler(recv);//设置接收处理函数
                return false;
